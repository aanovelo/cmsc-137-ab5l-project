...
\acmformat{Joseph Anthony C. Hermocilla, 2014.Peak Two Cloud: Scientific Computing
 on the Cloud.}
...
are familiar with SaaS as it provides user functionality, Google Docs 
...
\caption{Openstack at a glance\cite{sefraoui_openstack:_2012}.}
\label{fig:openstack}
\end{figure}
...
a working cluster on demand and terminate it after use. Each node in 
the cluster is 1 VCPU, 512MB RAM, and 5GB disk.

\subsection{Benchmarks}
In order to test the performance of P2C for scientific cloud computing, 
the Intel MPI Benchmarks(IMB) were executed on a vcluster deployed cluster.
IMB consists of several tests, however we selected one test for single 
transfer(PingPong), parallel transfer(Sendrecv), and collective 
operations(bcast). The test cluster has one master node and 9 slave nodes.
...
Figures~\ref{fig:pingpong_latency} to \ref{fig:bcast_latency} shows the 
results of running the IMB benchmarks.


\begin{figure}
\centerline{\includegraphics[width=10cm,keepaspectratio]{images/pingpong_latency}}
\caption{PingPong Latency results.}
\label{fig:pingpong_latency}
\end{figure}

\begin{figure}
\centerline{\includegraphics[width=10cm,keepaspectratio]{images/pingpong_thoughput}}
\caption{PingPong Throughput results.}
\label{fig:pingpong_throughput}
\end{figure}

\begin{figure}
\centerline{\includegraphics[width=10cm,keepaspectratio]{images/sendrecv_latency}}
\caption{SendRecv Latency results..}
\label{fig:sendrecv_latency}
\end{figure}


\begin{figure}
\centerline{\includegraphics[width=10cm,keepaspectratio]{images/sendrecv_throughput}}
\caption{SendRecv Throughput results.}
\label{fig:sendrecv_throughput}
\end{figure}


\begin{figure}
\centerline{\includegraphics[width=10cm,keepaspectratio]{images/bcast_latency}}
\caption{Bcast Latency results.}
\label{fig:bcast_latency}
\end{figure}
...
Peak Two Cloud. Using Openstack, we developed vcluster to deploy MPI 
