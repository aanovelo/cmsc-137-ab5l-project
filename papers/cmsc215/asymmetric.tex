...
$logarithms\: mod\: q$ where q is a prime number representing the
number of elements of a finite field. Users generate independent random
numbers $X_{i}$ from the set of integers $\left\{ 1,2,...,q-1\right\} $.
...
,a decryption procedure \textbf{D}, and a message \textbf{M}, a public
key cryptosystem have the following properties\cite{rivest_method_1978}:
\begin{enumerate}
\item Decrypting an encrypted \textbf{M} results to \textbf{M}. \textbf{D}(\textbf{E}(\textbf{M}))
= \textbf{M.}
...
\item If \textbf{M} is decrypted and then encrypted then \textbf{M} is the
result. \textbf{E}(\textbf{D}(\textbf{M})) = \textbf{M}.
\end{enumerate}
The encryption an decryption functions rely on a key such that the
